...
class GARCHModel(ARCHModel):
	...
	def sigma2_update(self, t):
		...
		sigma2_slice = np.clip(self.sigma2[t - np.arange(1, self.p + 1)], 1e-10, 1e10)
		self.sigma2[t] = (
			np.sum(params_slice * err_slice ** 2) +
			np.sum(self.params[self.p + 1 + self.q:] * sigma2_slice) +
			self.params[-1][0]  # extract the scalar value from the array
		)
		...
		for t in range(1, h):
			if t <= self.q:
				...
				forecast_values[t] = (
					...
					np.sum(self.params[self.p + 1 + self.q:] * np.clip(sigma2_slice, 0, 1e10), axis=0)[0] +
					self.params[-1][0]  # extract the scalar value from the array
				)
...
if __name__ == '__main__':
	data = np.random.normal(size=17)  # данные для модели, работает пока только 17
